...
def resize_dataset(video_idx, dsize, info_dict):
    ...
    for img_path in img_list:
        ...
        rs_img_path = img_path.replace("210x260px", dsize).replace("raw", "processed")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset-root",
        type=str,
        default="./data/SLR/raw/phoenix2014/phoenix-2014-multisigner",
    ...
    for md in mode:
        ...
        np.save(f"{args.save_root}/{md}_info.npy", information)
        # update the total gloss dict
        sign_dict_update(sign_dict, information)
        # generate groudtruth stm for evaluation
        generate_gt_stm(information, f"{args.save_root}/groundtruth-{md}.stm")
        # resize images
...
# # * Modyfied from https://github.com/ycmin95/VAC_CSLR/blob/main/preprocess/dataset_preprocess.py
...
# def resize_dataset(video_idx, dsize, info_dict):
#     info = info_dict[video_idx]
#     folder_path = info["folder"].replace("/1/", "/")
#     img_list = glob.glob(f"{info_dict['prefix']}/{folder_path}")
#     for img_path in img_list:
#         rs_img = resize_img(img_path, dsize=dsize)
#         rs_img_path = img_path.replace("210x260px", dsize).replace("raw", "processed")
#         rs_img_dir = os.path.dirname(rs_img_path)
...
#     mode = ["dev", "test", "train"]
#     sign_dict = dict()
#     if not os.path.exists(f"data/processed/{args.dataset}"):
#         os.makedirs(f"data/processed/{args.dataset}")
#     for md in mode:
#         # generate information dict
#         information = csv2dict(
#             f"{args.dataset_root}/{args.annotation_prefix.format(md)}", dataset_type=md
#         )
#         np.save(f"data/processed/{args.dataset}/{md}_info.npy", information)
#         # update the total gloss dict
#         sign_dict_update(sign_dict, information)
#         # generate groudtruth stm for evaluation
#         generate_gt_stm(
#             information,
#             f"data/processed/{args.dataset}/{args.dataset}-groundtruth-{md}.stm",
#         )
#         # resize images
#         video_index = np.arange(len(information) - 1)
#         print(f"Resize image to {args.output_res}")
#         if not os.path.exists(f"data/processed/{args.dataset}/{md}"):
#             os.makedirs(f"data/processed/{args.dataset}/{md}")
#         if args.process_image:
#             if args.multiprocessing:
#                 run_mp_cmd(
#                     10,
...
#     for idx, (key, value) in enumerate(sign_dict):
#         save_dict[key] = [idx + 1, value]
#     np.save(f"data/processed/{args.dataset}/gloss_dict.npy", save_dict)
